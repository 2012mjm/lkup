...
let self = module.exports = {
  getNotUse: (messageId, count) => {
    ...
    {
      const query = 'SELECT id, phone FROM `session` `s` \
  ...
  addNewNumber: (phone) => {
    return new Promise((resolve, reject) => {
      self.findByPhone(phone).then(res => {
        return reject('این شماره در سیستم وجود دارد!')
      }, err => {
        TelegramApiService.sendCode(phone).then(res => {
          if(res.status === 'send_code') {
            return resolve('کد تایید برای شما ارسال شد\nکد را برای من ارسال کنید.')
          }
  ...
  verifyNewNumber: (phone, code) => {
    return new Promise((resolve, reject) => {
      TelegramApiService.verifyCode(phone, code).then(res => {
        if(res.status === 'ok') {
          self.insert(phone).then(res => {
