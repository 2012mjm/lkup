      ...
      if(user.tgState === 'get_message' && user.tgStateParams !== null) {
        ...
        MessageService.update(JSON.parse(user.tgStateParams).message_id, {button: data}).then(message => {
          SessionService.getNotUse(message.id, 10000).then(sessions => {
            let inlineKeyboard = []
            const countLikeList = [5, 10, 30, 50]
            if(user.isAdmin) {
              countLikeList.push(500)
  ...
  startKeyboard: (isAdmin=false) => {
    ...
    if(isAdmin) {
      keyboard.reply_markup.keyboard.push([{
        text: sails.__('Add new post')
      }, {
        text: sails.__('Manage posts')
      }])
      keyboard.reply_markup.keyboard.push([{
        text: sails.__('Manage payment')
      }, {
        text: sails.__('Manage users')
      }])
      keyboard.reply_markup.keyboard.push([{
        text: sails.__('Change charge amount')
      }])
    }
