...
let self = module.exports = {
  ...
  getMessageFromChannel: (username, id) => {
    return new Promise((resolve, reject) => {
      rp({
        ...
        return resolve({json: json, replyMarkup: {inline_keyboard: newJson}})
      }).catch((err) => {
        return reject(err)
      })
    })
  ...
  sendCode: (phone) => {
    return new Promise((resolve, reject) => {
      rp({
        method: 'POST',
        uri: `${sails.config.params.tg_api_auth_url}/login`,
        formData: {
          phone: phone
        },
        json: true
  ...
  verifyCode: (phone, code) => {
    return new Promise((resolve, reject) => {
      rp({
        method: 'POST',
        uri: `${sails.config.params.tg_api_auth_url}/verify`,
        formData: {
          phone: phone,
          code: code
        },
