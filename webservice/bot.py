...
# python3 bot.py
# GET http://127.0.0.1:5000/message?session=mahsa&channel_username=chYaab&message_id=987
# POST http://127.0.0.1:5000/bot-callback -> session=mahsa, channel_username=chYaab, message_id=987, callback_data=channel_980_vote_5
...
def getCrawler(sessionName=None, sessionId=None):
	if sessionName is None:
		...
		if sessionDb is None: return None, None
		sessionName = sessionDb['phone']
		sessionId = sessionDb['id']

	crawler = Crawler('/home/mjm3d/likeup_bot/webservice/session/'+sessionName)
...
def getInfoChannel(crawler, username, sessionId=None):
	...
	if status is 'ok':
		...
		del crawler
		if sessionId is not None: db.updateBlockDateSession(sessionId, afterDate)
...
def message():
	print('start message')
	crawler, sessionId = getCrawler(request.args.get('session', None), request.args.get('session_id', None, int))
	if crawler is None:
		if sessionId is not None: db.inactiveSession(sessionId)
		return jsonify({'error': 'crawler not found'}), 500
	
	peer, error = getInfoChannel(crawler, request.args.get('channel_username'), sessionId)
	if error is not None:
		del crawler
		return jsonify({'error': error}), 500
		
	messageCount, messages = crawler.getHistoryChannel(peer.id, peer.access_hash, 1, (request.args.get('message_id', None, int)+1))
	msg = messages[0]

	forward = crawler.filterForward(msg.fwd_from)
	if forward is not None: forward = json.loads(forward)

	entities = crawler.filterEntities(msg.entities)
	if entities is not None: entities = json.loads(entities)

	reply_markup = crawler.filterReplyMarkup(msg.reply_markup)
	if reply_markup is not None: reply_markup = json.loads(reply_markup)
...
def botCallback():
	crawler, sessionId = getCrawler(request.form.get('session', None), request.args.get('session_id', None, int))
	if crawler is None:
		if sessionId is not None: db.inactiveSession(sessionId)
		return jsonify({'error': 'crawler not found'}), 500
	
	peer, error = getInfoChannel(crawler, request.form.get('channel_username'), sessionId)
	if error is not None:
		del crawler
		return jsonify({'error': error}), 500
	...
	try:
		res = crawler.client(GetBotCallbackAnswerRequest(
			game=False,
			peer=InputPeerChannel(channel_id=peer.id, access_hash=peer.access_hash),
