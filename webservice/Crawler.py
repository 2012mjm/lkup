...
class Crawler:
	...
	def __init__(self, storage_name, api_id=78380, api_hash='4e9377bdd268a8303ebb0102fd025cd4'):
		self.client = TelegramClient(storage_name, api_id, api_hash)
		try:
			print('client connect status', self.client.connect())
	...
	def authorized_step(self, phone=None):
		if self.client is None:
	...
	def authorized_verify(self, phone=None, code=None):
		if self.client is None:
			return 'client_none', None
		if not self.client.is_user_authorized():
			try:
				me = self.client.sign_in(phone, code)
			except PhoneNumberUnoccupiedError as err:
				me = self.client.sign_up(code, 'ali')
	...
	def getFulChannel(self, id, access_hash):
		...
		result = self.client(GetFullChannelRequest(channel=InputChannel(id, access_hash)))
	...
	def mediaPhoto(self, media, getDownload=False):
		...
		location = json.dumps({'volume_id':photoSize.location.volume_id, 'local_id':photoSize.location.local_id, 'secret':photoSize.location.secret})
		if getDownload: md5 = self.downloadAndMd5(media)
		else: md5 = None
		if media.photo.has_stickers is True: has_stickers = 1
		elif media.photo.has_stickers is None: has_stickers = 0
		else: has_stickers = 0
	...
	def mediaDocument(self, media, getDownload=False):
		location = json.dumps({'id':media.document.id, 'access_hash':media.document.access_hash, 'version':media.document.version})
	...
	def filterReplyMarkup(self, replyMarkup):
		if replyMarkup is None: return None
		rowList = []
		for row in replyMarkup.rows:
			buttonList = []
			for button in row.buttons:
				if isinstance(button, KeyboardButtonCallback): buttonList.append({'_': 'keyboardButtonCallback', 'text': button.text, 'callback_data': button.data.decode("utf-8")})
				elif isinstance(button, KeyboardButtonUrl): buttonList.append({'_': 'keyboardButtonUrl', 'text': button.text, 'url': button.url})
				elif isinstance(button, KeyboardButton): buttonList.append({'_': 'keyboardButton', 'text': button.text}) 
				elif isinstance(button, KeyboardButtonRequestPhone): buttonList.append({'_': 'keyboardButtonRequestPhone', 'text': button.text})
				elif isinstance(button, KeyboardButtonRequestGeoLocation): buttonList.append({'_': 'keyboardButtonRequestGeoLocation', 'text': button.text})
				elif isinstance(button, KeyboardButtonSwitchInline): buttonList.append({'_': 'keyboardButtonSwitchInline', 'same_peer': button.same_peer, 'text': button.text, 'query': button.query})
				elif isinstance(button, KeyboardButtonGame): buttonList.append({'_': 'keyboardButtonGame', 'text': button.text})
				elif isinstance(button, KeyboardButtonBuy): buttonList.append({'_': 'keyboardButtonBuy', 'text': button.text})
